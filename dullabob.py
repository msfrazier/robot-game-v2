    ...
    for bot in one_robots:
        if bot.player_id != this_robot.player_id:
            if bot.hp < weakest_enemy:
                ...
                if bot.hp <= 5 and (bot.location not in illegals) and (
...
def empty_score(this_robot, loc, game):
    ...
    # if we are trying to run away from spawn, non-spawn adjacent squares with no enemies by them are good
...
def pin_to_spawn(this_robot, game, illegals):
    ...
    for bot in one_robots:
        if bot.player_id != this_robot.player_id:
            loc2 = bot.location
            if spawn(loc2):
                if not_spawn(loc) and (loc not in illegals):
                    return ['guard']
    for bot in two_robots:
        if bot.player_id != this_robot.player_id:
            loc2 = bot.location
...
def tentative_act(this_robot, game, illegals):
    ...
    if verbose:
        print("current location: ", this_robot.location)
    if verbose > 1:
        print("bots 1 away: ", one_robots)
    if verbose > 1:
        print("bots 2 away: ", two_robots)
