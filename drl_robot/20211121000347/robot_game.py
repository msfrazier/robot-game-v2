...
class Robot(DRLRobot):
    ...
    def move(index):
        def f(game, robot):
            ...
            if 'obstacle' in game.loc_types(offsets[index]) or 'invalid' in game.loc_types(
                    offsets[index]):
                dist = 999
                location = 0
    ...
    def get_state(game, robot):
        ...
        freinds = np.array(
            [bool(game.robots[loc].hp) if Robot.enemy_at_loc(game, robot, loc) > 0 else 0 for loc in
             locs_around])
        enemies = np.array(
            [bool(game.robots[loc].hp) if Robot.enemy_at_loc(game, robot, loc) < 0 else 0 for loc in
             locs_around])
